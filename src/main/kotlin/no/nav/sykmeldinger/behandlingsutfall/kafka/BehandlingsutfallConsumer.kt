...
class BehandlingsutfallConsumer(
        ...
        GlobalScope.launch(Dispatchers.IO) {
            ...
            while (applicationState.ready) {
                try {
                    ...
                    log.info(
                        "Unsubscribed from topic ${environment.behandlingsutfallTopic} and waiting for 10 seconds before trying again"
                    )
                    delay(10_000)
                }
            }
            ...
            while (applicationState.ready) {
                ...
                if (!consumerRecords.isEmpty) {
                    totalDuration += measureTime {
                        ...
                        val behandlingsutfallRecrods =
                            consumerRecords.filterNot { it.value() == null }
    ...
    private suspend fun handleBehandlingsutfall(
        consumerRecords: List<ConsumerRecord<String, String>>
    ) =
        withContext(Dispatchers.IO) {
            val behandlingsutfalls =
                consumerRecords.map {
