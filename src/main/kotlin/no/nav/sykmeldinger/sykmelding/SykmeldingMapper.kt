...
class SykmeldingMapper private constructor() {
    ...
    companion object {
        ...
        private val diagnoserSomGirRedusertArbgiverPeriode =
            listOf("R991", "U071", "U072", "A23", "R992")
            ...
            return Sykmelding(
                ...
                arbeidsgiver =
                    Arbeidsgiver(
                        ...
                        stillingsprosent =
                            receivedSykmelding.sykmelding.arbeidsgiver.stillingsprosent,
                    ),
                sykmeldingsperioder =
                    receivedSykmelding.sykmelding.perioder.map {
                        it.toSykmeldingsPeriode(receivedSykmelding.sykmelding.id)
                    },
                medisinskVurdering =
                    if (skjermesForPasient) {
                        null
                    } else {
                        receivedSykmelding.sykmelding.medisinskVurdering.toMedisinskVurdering()
                    },
                ...
                utdypendeOpplysninger =
                    if (skjermesForPasient) {
                        ...
                        toUtdypendeOpplysninger(receivedSykmelding.sykmelding.utdypendeOpplysninger)
                    },
                ...
                tiltakNAV =
                    if (skjermesForPasient) {
                        ...
                        receivedSykmelding.sykmelding.tiltakNAV
                    },
                andreTiltak =
                    if (skjermesForPasient) {
                        null
                    } else {
                        receivedSykmelding.sykmelding.andreTiltak
                    },
                meldingTilNAV =
                    if (skjermesForPasient || receivedSykmelding.sykmelding.meldingTilNAV == null) {
                        null
                    } else {
                        MeldingTilNav(
                            bistandUmiddelbart =
                ...
                kontaktMedPasient =
                    KontaktMedPasient(
                        ...
                        begrunnelseIkkeKontakt =
                            receivedSykmelding.sykmelding.kontaktMedPasient.begrunnelseIkkeKontakt,
                    ),
                behandletTidspunkt =
                    receivedSykmelding.sykmelding.behandletTidspunkt.atOffset(ZoneOffset.UTC),
                ...
                papirsykmelding =
                    receivedSykmelding.sykmelding.avsenderSystem.navn == "Papirsykmelding",
                harRedusertArbeidsgiverperiode =
                    receivedSykmelding.sykmelding.medisinskVurdering
                        .getHarRedusertArbeidsgiverperiode(receivedSykmelding.sykmelding.perioder),
                merknader =
                    receivedSykmelding.merknader?.map {
                        Merknad(beskrivelse = it.beskrivelse, type = it.type)
                    },
                rulesetVersion = receivedSykmelding.rulesetVersion,
                utenlandskSykmelding =
                    receivedSykmelding.utenlandskSykmelding?.let { UtenlandskSykmelding(it.land) },
            )
        }
        ...
        private fun toAktivietIkkeMulig(
            it: no.nav.syfo.model.AktivitetIkkeMulig
        ): AktivitetIkkeMulig {
        ...
        private fun no.nav.syfo.model.ArbeidsrelatertArsak.toArbeidsrelatertArsakDto():
        ...
        private fun toArbeidsrelatertArsakType(
            arbeidsrelatertArsakType: no.nav.syfo.model.ArbeidsrelatertArsakType
        ): ArbeidsrelatertArsakType {
            return when (arbeidsrelatertArsakType) {
                no.nav.syfo.model.ArbeidsrelatertArsakType.MANGLENDE_TILRETTELEGGING ->
        ...
        private fun toMedisinskArsakTypeDto(
            medisinskArsakType: no.nav.syfo.model.MedisinskArsakType
        ): MedisinskArsakType {
            return when (medisinskArsakType) {
                no.nav.syfo.model.MedisinskArsakType.AKTIVITET_FORHINDRER_BEDRING ->
                    MedisinskArsakType.AKTIVITET_FORHINDRER_BEDRING
                no.nav.syfo.model.MedisinskArsakType.AKTIVITET_FORVERRER_TILSTAND ->
                    MedisinskArsakType.AKTIVITET_FORVERRER_TILSTAND
                no.nav.syfo.model.MedisinskArsakType.TILSTAND_HINDRER_AKTIVITET ->
        ...
        private fun toBehandler(receivedSykmelding: ReceivedSykmelding) =
            Behandler(
        ...
        private fun no.nav.syfo.model.MedisinskVurdering.getHarRedusertArbeidsgiverperiode(
        ...
        private fun no.nav.syfo.model.MedisinskVurdering.checkSmittefare() =
            annenFraversArsak?.grunn?.any { annenFraverGrunn ->
                annenFraverGrunn == no.nav.syfo.model.AnnenFraverGrunn.SMITTEFARE
            } == true
        ...
        private fun toUtdypendeOpplysninger(
            utdypendeOpplysninger: Map<String, Map<String, no.nav.syfo.model.SporsmalSvar>>
        ): Map<String, Map<String, SporsmalSvar>> {
            return utdypendeOpplysninger.mapValues {
                it.value
                    .mapValues { entry -> entry.value.toSporsmalSvar() }
                    .filterValues { sporsmalSvar ->
                        !sporsmalSvar.restriksjoner.contains(SvarRestriksjon.SKJERMET_FOR_PASIENT)
                    }
            }
        }
        ...
        private fun no.nav.syfo.model.SvarRestriksjon.toSvarRestriksjon(): SvarRestriksjon {
            return when (this) {
                no.nav.syfo.model.SvarRestriksjon.SKJERMET_FOR_ARBEIDSGIVER ->
                    SvarRestriksjon.SKJERMET_FOR_ARBEIDSGIVER
                no.nav.syfo.model.SvarRestriksjon.SKJERMET_FOR_NAV ->
                    SvarRestriksjon.SKJERMET_FOR_NAV
                no.nav.syfo.model.SvarRestriksjon.SKJERMET_FOR_PASIENT ->
                    SvarRestriksjon.SKJERMET_FOR_PASIENT
            }
        }

        private fun no.nav.syfo.model.MedisinskVurdering.toMedisinskVurdering():
            MedisinskVurdering {
        ...
        private fun no.nav.syfo.model.AnnenFraverGrunn.toAnnenFraverGrunn(): AnnenFraverGrunn {
            return when (this) {
                ...
                no.nav.syfo.model.AnnenFraverGrunn.ARBEIDSRETTET_TILTAK ->
                    AnnenFraverGrunn.ARBEIDSRETTET_TILTAK
                no.nav.syfo.model.AnnenFraverGrunn.BEHANDLING_FORHINDRER_ARBEID ->
                    AnnenFraverGrunn.BEHANDLING_FORHINDRER_ARBEID
                no.nav.syfo.model.AnnenFraverGrunn.BEHANDLING_STERILISERING ->
                    AnnenFraverGrunn.BEHANDLING_STERILISERING
                no.nav.syfo.model.AnnenFraverGrunn.DONOR -> AnnenFraverGrunn.DONOR
                no.nav.syfo.model.AnnenFraverGrunn.GODKJENT_HELSEINSTITUSJON ->
                    AnnenFraverGrunn.GODKJENT_HELSEINSTITUSJON
                no.nav.syfo.model.AnnenFraverGrunn.MOTTAR_TILSKUDD_GRUNNET_HELSETILSTAND ->
                    AnnenFraverGrunn.MOTTAR_TILSKUDD_GRUNNET_HELSETILSTAND
                no.nav.syfo.model.AnnenFraverGrunn.NODVENDIG_KONTROLLUNDENRSOKELSE ->
                ...
                no.nav.syfo.model.AnnenFraverGrunn.UFOR_GRUNNET_BARNLOSHET ->
                    AnnenFraverGrunn.UFOR_GRUNNET_BARNLOSHET
            }
        }
    }
}
